...
The stack uses an `x86_64` guest image because the Linux x64 emulator cannot boot an `arm64-v8a` system image on an x86_64 host. API 36 Google APIs images can expose translated ARM64 support, so the default is suitable for AI APKs that only bundle `arm64-v8a` model libraries as well as APKs that bundle real `x86_64` AI libraries.
...
* If the APK has AI native libraries only for `arm64-v8a` and the emulator image exposes translated `arm64-v8a`, the bridge installs with `--abi arm64-v8a`.
