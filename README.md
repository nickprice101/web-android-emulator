...
The current custom bridge lives in `bridge-webrtc/` and is intentionally scaffolded in `stub` mode first. That means:

* TURN credentials are generated by the custom bridge
* the frontend now targets `/bridge/...` for custom WebRTC mode
* the media capture pipeline is not implemented yet
